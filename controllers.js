...
const controllers = {
    registerUser: async (req, res) => {
        ...
        if (userExists) { return res.status(400).json({ message: 'Utilizador ou email já registado.' }); }
        const salt = await bcrypt.genSalt(10);
        const hashedPassword = await bcrypt.hash(password, salt);
        try {
            const user = await User.create({ username, email: email.toLowerCase(), password: hashedPassword, });
            res.status(201).json({ _id: user._id, username: user.username, avatar: user.avatar, token: generateToken(user._id), });
        } catch (error) { res.status(400).json({ message: 'Dados de utilizador inválidos.' }); }
    },
    loginUser: async (req, res) => {
        const { email, password } = req.body;
        const user = await User.findOne({ email: email.toLowerCase() });
        if (user && (await bcrypt.compare(password, user.password))) {
            if (user.isBlocked) { return res.status(403).json({ message: 'Esta conta encontra-se bloqueada.' }); }
            res.json({ _id: user._id, username: user.username, email: user.email, avatar: user.avatar, role: user.role, token: generateToken(user._id), });
    ...
    updatePassword: async (req, res) => {
        ...
        if (!oldPassword || !newPassword || newPassword.length < 6) { return res.status(400).json({ message: 'Dados inválidos fornecidos.'}); }
        const user = await User.findById(req.user.id);
        ...
        } else { res.status(401).json({ message: 'Senha antiga incorreta.' }); }
    },
    uploadAvatar: async (req, res) => {
        ...
        try {
            const user = await User.findById(req.user.id);
            if (!user) return res.status(404).json({ message: 'Utilizador não encontrado' });
            if (user.avatar && user.avatar.public_id !== 'default_avatar_id') { await cloudinary.uploader.destroy(user.avatar.public_id); }
            const result = await cloudinary.uploader.upload(req.file.path, { folder: 'brainskill_avatars', width: 150, height: 150, crop: 'fill', });
            user.avatar = { public_id: result.public_id, url: result.secure_url };
            await user.save();
            res.status(200).json({ url: result.secure_url });
        } catch (error) { res.status(500).json({ message: 'Falha no upload da imagem.' }); }
    },
    getRanking: async (req, res) => {
        const ranking = await User.find({ role: 'user' }).sort({ 'stats.wins': -1, 'stats.losses': 1, 'createdAt': 1 }).select('username avatar stats createdAt');
        res.status(200).json(ranking);
    },
    createDeposit: async (req, res) => {
        const { amount, method, proofText } = req.body;
        if (!amount || !method || (!proofText && !req.file)) { return res.status(400).json({ message: 'Dados insuficientes para o depósito.' }); }
        if (Number(amount) < config.minDeposit) { return res.status(400).json({ message: `O depósito mínimo é de ${config.minDeposit} MT.` }); }
        ...
        if (req.file) {
            ...
            } catch(e) { return res.status(500).json({ message: 'Erro ao carregar o comprovativo.'}); }
        }
        try {
            const transaction = await Transaction.create({ userId: req.user.id, type: 'deposit', amount: Number(amount), method, proof: proofData, });
            res.status(201).json(transaction);
        } catch(e) { res.status(500).json({ message: 'Erro ao criar a transação.' }); }
    },
    createWithdrawal: async (req, res) => {
        const { amount, method, holderName, phoneNumber } = req.body;
        const user = await User.findById(req.user.id);
        if (!amount || !method || !holderName || !phoneNumber) { return res.status(400).json({ message: 'Por favor, preencha todos os campos.' }); }
        if (user.balance < Number(amount)) { return res.status(400).json({ message: 'Saldo insuficiente.' }); }
        user.balance -= Number(amount);
        try {
            const transaction = await Transaction.create({ userId: req.user.id, type: 'withdrawal', amount: Number(amount), method, holderName, phoneNumber });
            await user.save();
            res.status(201).json(transaction);
        } catch(e) {
            user.balance += Number(amount);
    ...
    getMyGames: async (req, res) => {
        const games = await Game.find({ players: req.user.id, hiddenBy: { $ne: req.user.id } }).populate('players', 'username avatar').populate('winner', 'username').sort({ createdAt: -1 });
    ...
    hideGameFromHistory: async (req, res) => {
        try {
            ...
            if (!game.players.includes(req.user.id)) { return res.status(403).json({ message: 'Você não tem permissão para alterar esta partida.' }); }
            const nonRemovableStatus = ['waiting', 'in_progress'];
            if (nonRemovableStatus.includes(game.status)) { return res.status(400).json({ message: 'Não é possível remover uma partida em andamento.' }); }
            if (!game.hiddenBy.includes(req.user.id)) { game.hiddenBy.push(req.user.id); await game.save(); }
            res.status(200).json({ message: 'Partida removida do histórico com sucesso.' });
        } catch (error) { res.status(500).json({ message: 'Erro no servidor ao tentar remover a partida.' }); }
    },
    ...
    processTransaction: async (req, res) => {
        ...
        if (!transaction || transaction.status !== 'pending') { return res.status(404).json({ message: 'Transação não encontrada ou já processada.' }); }
        ...
        } else { return res.status(400).json({ message: 'Status inválido.' }); }
    ...
    getPlatformSettings: (req, res) => { res.json(config); },
    updatePlatformSettings: async (req, res) => {
        // ... (Lógica para atualizar as configurações)
    },
    getPaymentMethodsAdmin: (req, res) => { res.json(config.paymentMethods); },
    updatePaymentMethods: (req, res) => {
        // ... (Lógica para atualizar os métodos de pagamento)
    }
};
